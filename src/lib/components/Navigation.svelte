<script lang="ts">
	...
	import { searchContent, type SearchableItem, getSlugFromModuleId } from '$lib/helpers/constants';
			...
			<div class="hidden md:block flex-1 max-w-lg mx-8">
				<div class="relative">
					...
					{#if showSearchResults}
						<div class="absolute top-full right-0 left-0 z-50 mt-2 max-h-96 overflow-y-auto rounded-xl border border-gray-200 bg-white shadow-xl dark:border-gray-600 dark:bg-gray-800">
							{#if isSearching}
							...
							{:else}
								<div class="px-4 py-4 text-center text-gray-500 dark:text-gray-400">
									<svg class="mx-auto h-8 w-8 mb-2 text-gray-300 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
		...
		{#if showMobileSearch}
			<div class="md:hidden pb-4 animate-in slide-in-from-top-2 duration-200">
				...
				{#if showSearchResults && searchQuery.length >= 2}
					<div class="mt-2 max-h-80 overflow-y-auto rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-600 dark:bg-gray-800">
						{#if isSearching}
						...
						{:else if searchResults.length > 0}
							{#each searchResults as result}
								...
								>
									...
									<div class="text-xs text-blue-600 dark:text-blue-400">{result.type}</div>
									{#if result.preview}
